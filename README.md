...
```mermaid
flowchart TD
    A["Your embedder output"] --> B["TCN_encoders/{modality}/encoder.py<br/>BufferedEncoder — timing + buffering"]
    B --> C["TFN/low_rank_tucker.py<br/>cross-modal interaction (Tucker R=8)"]
    C --> D["predictive_models/{modality}/<br/>YOUR trained model lives here"]
    D --> E["poe/poe.py<br/>combines 4 model predictions"]
    E --> F["ema/ema.py<br/>temporal smoothing"]
    F --> G["output<br/>{global: (B,11), per_model: [(B,12)] × 4}"]

    classDef locked fill:#3a2233,stroke:#c0567a,color:#f5d7e3;
    classDef yours fill:#1f3a2e,stroke:#4caf80,color:#d7f5e6;
    class B,C,E,F locked;
    class A,D yours;
```

The fusion model runs at **1Hz** — one forward pass per second. Every component must produce its output within that budget.

> Pink = locked infrastructure (do not edit). Green = your work.
...
flowchart LR
    ...
    ROOT --> FM["fusion_model.py"]
    ROOT --> TFN["TFN/<br/>tfn.py · low_rank_tucker.py"]
    ROOT --> POE["poe/<br/>poe.py"]
    ROOT --> EMA["ema/<br/>ema.py"]
    ROOT --> TCN["TCN_encoders/<br/>buffered_encoder · fixed_tcn · configs<br/>mouse · keyboard · notif · switching"]
    ROOT --> PRE["pre_embedders/<br/>mouse · keyboard · notif · switching"]
    ROOT --> PM["predictive_models/<br/>base.py · registry"]

    PM --> PMM["mouse/ keyboard/ notif/ switching/"]
    PMM --> SWAP["__init__.py"]
    PMM --> DUM["dummy.py"]
    PMM --> MOD["v1_your_model.py"]

    classDef locked fill:#3a2233,stroke:#c0567a,color:#f5d7e3;
    classDef yours fill:#1f3a2e,stroke:#4caf80,color:#d7f5e6;
    classDef swap fill:#3a3522,stroke:#c0a056,color:#f5ecd7;

    class FM,TFN,POE,EMA,TCN,DUM locked;
    class PRE,MOD yours;
    class SWAP swap;
```

> 🟥 Locked · 🟨 Swap point (change one import line) · 🟩 Your work
...
```mermaid
flowchart LR
    subgraph SRC["Raw inputs (your modules)"]
        M["mouse<br/>(1,64) every 1s"]
        K["keyboard<br/>(64,) or None"]
        N["notif<br/>dict or None / 5s"]
        S["switching<br/>dict or None / 120s"]
    end

    subgraph ENC["BufferedEncoders — .step()"]
        ME["mouse_enc"]
        KE["keyboard_enc"]
        NE["notif_enc"]
        SE["switching_enc"]
    ...
    M --> ME --> EM["(emb, freshness)"]
    K --> KE --> EK["(emb, freshness)"]
    N --> NE --> EN["(emb, freshness)"]
    S --> SE --> ES["(emb, freshness)"]

    EM --> FUS["inferrer_fusion([...])"]
    EK --> FUS
    EN --> FUS
    ES --> FUS
    FUS --> OUT["output dict"]
    ...
    classDef locked fill:#3a2233,stroke:#c0567a,color:#f5d7e3;
    class ME,KE,NE,SE,FUS locked;
```

> Note the differing cadences: mouse fires every tick, while switching may hold a stale embedding for up to 120s (staleness decays its contribution via `exp(-staleness/60)`).
...
```mermaid
flowchart LR
    IN["TFN slice<br/>(B, 512)"] --> MODEL["Your model<br/>forward()"]
    MODEL --> OUT["(B, 12)"]

    OUT --> F["dims 0–4<br/>factor scores<br/>MD · TD · EF · FR · AR"]
    OUT --> L["dims 5–9<br/>RAW state logits<br/>Flow · Neutral · Bored<br/>Distracted · Overloaded"]
    OUT --> H["dim 10<br/>H_norm ∈ [0,1]"]
    OUT --> MM["dim 11<br/>M (top1 − top2)"]

    classDef warn fill:#3a2233,stroke:#c0567a,color:#f5d7e3;
    classDef calc fill:#22303a,stroke:#56a0c0,color:#d7ecf5;
    class L warn;
    class H,MM calc;
```

> ⚠️ dims 5–9 stay **raw** (no softmax). dims 10–11 must come from `compute_uncertainty()`.
...
inferrer = InferrerFusion(d_dims=[64, 64, 32, 64], rank=4)   # was 8, try 4
```
...
```mermaid
flowchart TD
    L1["Level 1<br/>Offline F1 / MCC / confusion matrix"] --> L2["Level 2<br/>Temporal validity<br/>(no shuffled windows)"]
    L2 --> L3["Level 3<br/>LOSO cross-validation<br/>(strict — no window overlap across splits)"]
    L3 --> L4["Level 4<br/>Calibration check"]
    L4 --> L5["Level 5<br/>Ablation table"]
```
...
```mermaid
flowchart LR
    HCI["HCI inputs"] --> P1["Path 1<br/>fusion model"]
    HCI --> P2["Path 2<br/>biosignal proxy"]

    P1 --> S1["cognitive state<br/>predictions"]
    P2 --> BIO["predicted biosignals<br/>EDA · ECG"] --> S2["cognitive state"]

    S1 -. "KL divergence loss (training only)" .- S2

    classDef ext fill:#2a2a3a,stroke:#7a7ac0,color:#dcdcf5;
    class P2,BIO,S2 ext;
```

> At inference time **only Path 1 runs** — no biosensor hardware required. Path 2 is training-time regularization only.

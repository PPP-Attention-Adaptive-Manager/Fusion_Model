"""
train_switching_models.py
=========================
Trains and evaluates the switching modality GRU model.

Usage:
    cd ~/fusion_model
    python train_switching_models.py \
        --sequences  ~/tucker_outputs/sequences.npy \
        --output_dir predictive_models/switching/weights \
        --epochs 50
...
MODALITY_IDX = 3          # switching slot index in Tucker tensor
...
# ═════════════════════════════════════════════════════════════════════════════
# Label derivation  (identical to keyboard / mouse)
# ═════════════════════════════════════════════════════════════════════════════
...
def train_loso(
    ...
    for test_user in unique_users:
        ...
        for epoch in range(epochs):
            train_loss = train_one_epoch(
                model, train_seqs, optimizer, class_weights
            )
    ...
    metrics = compute_metrics(
        all_true, all_pred, f"{model_name} — LOSO overall"
    )
...
def main():
    ...
    parser.add_argument("--output_dir", type=str,
        default=str(FUSION_DIR / "predictive_models" / "switching" / "weights"))
